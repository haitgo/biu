...
type Session struct {
	IdKey      string
	CookieAge  int64  //生存时间，单位：秒
	SessionAge int64  //sesion生存时间
...
func (self Session) Use() cache.Cacher {
	...
	o.cache.Expire(o.SessionAge)
	return o.cache
}

//使用session时使用
func (self Session) Init(r *http.Request) cache.Cacher {
	o := &self
	o.Request = r
	sid := o.getCookieSessionId()
	o.initCache(sid)
...
func (this *Session) createSessionId() string {
	...
	cookie := http.Cookie{Name: SESSION_ID_KEY, Value: url.QueryEscape(sid), Path: "/", HttpOnly: true, MaxAge: int(this.CookieAge)}
